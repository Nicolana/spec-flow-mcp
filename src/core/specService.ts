...
export async function getdevelopmentSpec(request: SpecRequest): Promise<SpecResponse> {
  ...
  logger.info(`获取规范: ${spec_name} (${category}) - 项目路径: ${projectRoot}`);
  ...
  try {
    ...
    const content = await readSpecFile(spec_name, category, projectRoot);
    const filePath = getSpecFilePath(spec_name, category, projectRoot);
...
export async function createDevelopmentSpec(request: CreateSpecRequest): Promise<SpecOperationResponse> {
  const { spec_name, content, category = 'frontend', projectRoot } = request;
  
  logger.info(`创建规范: ${spec_name} (${category}) - 项目路径: ${projectRoot}`);
  ...
  try {
    ...
    // 创建规范文件
    await writeSpecFile(spec_name, content, category, projectRoot);
...
export async function editDevelopmentSpec(request: EditSpecRequest): Promise<SpecOperationResponse> {
  ...
  logger.info(`编辑规范: ${spec_name} (${category}) - 项目路径: ${projectRoot}`);
