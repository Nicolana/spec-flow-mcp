...
export async function getdevelopmentSpec(request: SpecRequest): Promise<SpecResponse> {
  const { spec_name, projectRoot } = request;
  
  logger.info(`获取规范: ${spec_name} - 项目路径: ${projectRoot}`);
  ...
  try {
    ...
    const content = await readSpecFile(spec_name, projectRoot);
    const filePath = getSpecFilePath(spec_name, projectRoot);
...
export async function createDevelopmentSpec(request: CreateSpecRequest): Promise<SpecOperationResponse> {
  const { spec_name, content, projectRoot } = request;
  
  logger.info(`创建规范: ${spec_name} - 项目路径: ${projectRoot}`);
  ...
  try {
    ...
    // 检查规范是否已存在
    if (await specFileExists(spec_name, projectRoot)) {
      return {
        success: false,
    ...
    // 创建规范文件
    await writeSpecFile(spec_name, content, projectRoot);
    
    logger.info(`成功创建规范: ${spec_name}`);
...
export async function editDevelopmentSpec(request: EditSpecRequest): Promise<SpecOperationResponse> {
  ...
  logger.info(`编辑规范: ${spec_name} - 项目路径: ${projectRoot}`);
...
export async function deleteDevelopmentSpec(request: DeleteSpecRequest): Promise<SpecOperationResponse> {
  ...
  logger.info(`删除规范: ${spec_name} - 项目路径: ${projectRoot}`);
  ...
  try {
    ...
    // 删除规范文件
    await deleteSpecFile(spec_name, projectRoot);
    
    logger.info(`成功删除规范: ${spec_name}`);
 ...
 * 列出所有可用的开发规范
 */
export async function listAvailableSpecs(request: ListSpecsRequest): Promise<ListSpecsResponse> {
  const { projectRoot } = request;
  
  logger.info(`获取规范列表 - 项目路径: ${projectRoot}`);
  ...
  try {
    ...
    const specs = await listSpecFiles(projectRoot);
